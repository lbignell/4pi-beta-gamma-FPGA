...
while i<max(size(Data(:,1)))
    ...
    %delayed event...
        if (Data(i,3)*TickLength)>DelayedEvents(1,3)
            %DON'T iterate the counter!
            %Process the delayed event.
            Ch1 = DelayedEvents(1,1);
            Ch2 = DelayedEvents(1,2);
            ThisTime = DelayedEvents(1,3);
            %don't actually use 4th row...
            if(min(size(DelayedEvents))==1)
                %unset the delayed events flag...
                hazDelayedEvents = false;
                %disp(sprintf('hazDelayedEvents set to FALSE'))
                %disp(sprintf('DelayedEvents; Ch1=%i, Ch2=%i, ticks=%d',...
                %    DelayedEvents(1,1), DelayedEvents(1,2), DelayedEvents(1,3)));
            else
                %Delete the 1st row as it has been processed.
                DelayedEvents(1,:) = [];
                %disp('Just Deleted first row of DelayedEvents')
                %DelayedEvents
            end
        else
            Ch1 = Data(i,1);
            ...
            %times.
                ...
                Ch1 = 0;%for the event to be processed.
                    %Append a line
                    [rows,cols] = size(DelayedEvents);
                    DelayedEvents(rows+1,1) = true;
                    DelayedEvents(rows+1,2) = false;
                    DelayedEvents(rows+1,3) = ThisTime + Ch1Delay;
                    DelayedEvents(rows+1,4) = Data(i,4);
%                end
            end
        ...
        %times.
            ...
            Ch1 = 0;%for the event to be processed.
                ...
                DelayedEvents(1,1) = true;%always Ch1
                DelayedEvents(1,2) = false;%Never Ch2
                DelayedEvents(1,3) = ThisTime + Ch1Delay;
                DelayedEvents(1,4) = Data(i,4);
