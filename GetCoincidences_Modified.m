    ...
    %delayed event...
        if Data(i,3)>DelayedEvents(1,3)
            ...
            %times.
                ...
                if (min(size(DelayedEvents))==1)
                    %Overwrite initialised value
                    DelayedEvents(1,1) = 1;%always Ch1
                    DelayedEvents(1,2) = 0;%Never Ch2
                    DelayedEvents(1,3) = ThisTime + Ch1Delay;
                    DelayedEvents(1,4) = Data(i,4);
                else
                    %Append a line
                    [rows,cols] = size(DelayedEvents);
                    DelayedEvents(rows+1,1) = 1;
                    DelayedEvents(rows+1,2) = 0;
